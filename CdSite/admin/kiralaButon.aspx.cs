...
namespace CdSite
{
    public partial class kiralaButon : System.Web.UI.Page
    {
        public int _film_id { get { return Session["kiralanacak_id"] == null ? 0 : int.Parse(Session["kiralanacak_id"].ToString()); } }
        ...
        {
            var kontrol = _dc.tx_filmKiras.FirstOrDefault(k => k.kullanici_id.Equals(Session["this_id"]) && k.film_id == _film_id);
            var film = _dc.dt_films.FirstOrDefault(g => g.id == _film_id);

            if (kontrol == null)
            {
                tx_filmKira ekle = new tx_filmKira();
                ekle.kullanici_id = Convert.ToInt32(Session["this_id"]);
        ...
        protected void kullanicilar_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Session["this_id"] = kullanicilar.DataKeys[index].Value;
        }
    }
}